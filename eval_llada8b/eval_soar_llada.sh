...
for length in "${lengths[@]}"; do
    python $script --tasks humaneval --model llada_dist \
        ...
        --trust_remote_code \
        --model_args model_path='GSAI-ML/LLaDA-8B-Base',gen_length=$length,steps=$length,block_length=$length,low_cpu_mem_usage=True,device_map='auto',torch_dtype=torch.bfloat16,enable_soar=True \
