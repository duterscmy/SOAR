...
class DreamGenerationMixin:
    ...
    def diffusion_generate(
        ...
        **kwargs,
        ...
        elif decode_method == "soar":
            result = self._sample_soar(
                input_ids,
                attention_mask=attention_mask,
                generation_config=generation_config,
                generation_tokens_hook_func=generation_tokens_hook_func,
        ...
        elif decode_method == "pbs":
            result = self._sample_pbs(
                input_ids,
                attention_mask=attention_mask,
                generation_config=generation_config,
                generation_tokens_hook_func=generation_tokens_hook_func,
    ...
    def _sample_soar(
        ...
        # soar params
        max_beam_size = generation_config.beam_size
        entropy_threshhold = generation_config.threshhold
        min_high_confidence_len = generation_config.min_parallel_tokens
        max_high_confidence_len = generation_config.max_parallel_tokens
        ...
        while True:
            ...
            for seq_idx, (seq, cumulative_avg_prob, num_unmasked_tokens) in enumerate(beam):
                ...
                # switch parallel decoding or position beam search
                if len(high_confidence_indices) >= min_high_confidence_len:  # parallel decoding
                    ...
                    total_prob = cumulative_avg_prob * num_unmasked_tokens
            ...
            # sort candidates by average confidence
    ...
    def _sample_pbs(
        ...
        print(f"========PBS, beam_size={beam_size},max_length={max_length},steps={steps},alg={alg},temperature={temperature}==========", flush=True)
        histories = [] if (return_dict_in_generate and output_history) else None
        ...
        # init beam: [(sequence, cumulative_log_prob)]
        beam = [(x.clone(), 0.0)]
